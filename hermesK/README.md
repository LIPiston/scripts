...
- Windows：先把 Git Bash 的 `/c/Users/...` 转换为真实 Windows 路径，再让 tmux 在该目录启动 `powershell.exe` 并运行 `hermes`。
- Linux/macOS：在 tmux 会话中直接从用户主目录运行 `hermes`。
- 如果 tmux 会话 `hermesK` 已存在：直接进入该会话，不重复启动 Hermes。
- 如果当前已经在 tmux 内：切换到 `hermesK` 会话。
- 如果当前不在 tmux 内：附加到 `hermesK` 会话。

Windows 使用 PowerShell 启动，是为了避免 Git Bash/MSYS 在启动 Hermes 时错误转换 Windows 盘符路径，产生类似 `C:\c`、`D:\d` 之类的错误工作目录或垃圾目录。
