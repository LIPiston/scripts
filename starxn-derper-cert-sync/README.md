...
3. 比较来源和 `data` 目录中的证书/私钥；
4. 如果无变化则直接退出，不重启容器；
5. 如果有变化，在 `data/cert-sync-backups/<时间戳>/` 保存旧文件；
6. 使用临时文件和原子替换更新目标文件；
7. 校验来源与目标 SHA-256 一致；
8. 重启 `derper` Compose 服务；
9. 从容器内验证证书可读且可解析；
10. 检查公网 `https://derper.lipiston.eu.org/generate_204` 返回 `204`。
